...
def calculate_footprint_imbalance(df: pd.DataFrame, imbalance_ratio: float = 3.0, window: int = 100) -> pd.Series:
    ...
    Особенности реализации:
    Эта версия полностью векторизована и оптимизирована по памяти.
    1.  **Векторизованное определение агрессора**: Вместо `.apply()` используется
        векторизованный доступ к данным стакана (`.str[0].str[0]`) для
        определения агрессора в каждой сделке.
    2.  **Эффективное сопоставление**: Вместо ресурсоемкого `pd.merge()`
        используется `groupby().sum().map()` для сопоставления каждой сделке
        агрегированного объема на предыдущем ценовом уровне. Это значительно
        снижает потребление памяти.
    3.  **Фокус на бычьем дисбалансе**: Как и в оригинальной версии, для
        простоты рассчитывается только дисбаланс на покупку.
    ...
    # Рассчитываем объемы покупок и продаж
    buy_vol = df_filled['quantity'].where(is_buy, 0)
    sell_vol = df_filled['quantity'].where(is_sell, 0)

    # --- 2. Эффективный расчет диагонального дисбаланса ---
    # Создаем временный DataFrame только с нужными данными
    ...
    # Группируем, чтобы получить суммарный объем продаж на каждом ценовом уровне.
    # Это единственный шаг, который требует агрегации по всему датасету.
    price_level_vols = footprint_df.groupby('price')[['sell_vol']].sum()

    # Сдвигаем, чтобы получить объем продаж на ПРЕДЫДУЩЕМ ценовом уровне
    price_level_vols['prev_sell_vol'] = price_level_vols['sell_vol'].shift(1).fillna(0)

    # Создаем словарь-мэппинг: {цена -> объем продаж на предыдущем уровне}
    # Это намного эффективнее по памяти, чем pd.merge
    price_to_prev_sell_vol_map = price_level_vols['prev_sell_vol']

    # Применяем мэппинг к каждой сделке
    prev_sell_vol_series = footprint_df['price'].map(price_to_prev_sell_vol_map).fillna(0)

    # --- 3. Расчет бычьего дисбаланса ---
    # Сравниваем объем покупки КАЖДОЙ СДЕЛКИ с агрегированным объемом продаж на уровне ниже
    buy_imbalance = (footprint_df['buy_vol'] > prev_sell_vol_series * imbalance_ratio).astype(int)

    # --- 4. Финальный результат ---
    # Как и в оригинале, считаем только бычий дисбаланс и берем скользящую сумму
    net_imbalance = buy_imbalance
